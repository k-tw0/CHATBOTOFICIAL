...
export class MessageService {
  ...
  sendMessage(message, category) {
    ...
    if (similarQuestion) {
      const { respuesta, codigo, videos, pasos } = similarQuestion;
      const videoURL = videos && videos.length > 0 ? videos : null;

      if (videoURL) {
        const answerElement = createAnswerElement(respuesta, codigo, videoURL, pasos);
        messageContainer.appendChild(answerElement);
      } else {
        const answerElement = createAnswerElement(respuesta, codigo, null, pasos);
        messageContainer.appendChild(answerElement);
      }
    } else {
...
function createAnswerElement(response, codigo, videos, pasos) {
  ...
  setTimeout(() => {
    ...
    // Aplicar estilos y contenido a la respuesta
    applyStyles(answerElement, parsedResponse);
  

    if (currentCategory === "Aprende facil") {
    ...
    if (currentCategory === "Pasos y operaciones") {
      if (pasos && pasos.length > 0) {
        ...
        pasos.forEach((paso) => {
          ...
          // Agregar evento de clic al elemento console-pasos
          if (consolePasosElement) {
            consolePasosElement.addEventListener("click", () => {
              // Crear el mensaje de copiado
              const copyMessage = document.createElement("div");
              copyMessage.textContent = "Texto copiado";
              copyMessage.classList.add("copy-message");
              consolePasosElement.appendChild(copyMessage);
          
              // Copiar el texto al hacer clic en el mensaje de copiado
              copyMessage.addEventListener("click", (event) => {
                event.stopPropagation();
                const copiedText = consolePasosElement.textContent.replace(/\/C(.*?)C\//g, "$1");
                navigator.clipboard.writeText(copiedText);
              });
          
              // Desaparecer el mensaje de copiado después de 3 segundos
              setTimeout(() => {
        ...
        answerElement.appendChild(stepsElement);
      }
    }
    

    if (currentCategory === "Pasos y operaciones" && videos) {
...
function parseSpecialStyles(response) {
  ...
  parsedResponse = parsedResponse.replace(/\/L(.*?)L\//g, "<a href='$1'>$1</a>"); // enlace

   // Reemplazar los estilos especiales definidos en el JSON
   parsedResponse = parsedResponse.replace(/\/C(.*?)C\//g, "<span class='console-pasos'>$1</span>"); // estilo de consola

  return parsedResponse;
}

// Función para aplicar estilos y contenido a la respuesta
function applyStyles(answerElement, response) {
